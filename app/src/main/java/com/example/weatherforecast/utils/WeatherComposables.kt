...
object WeatherComposables {
    ...
    @Composable
    fun updateMinMaxPressureValue(minMaxPressure: Int, pressureOption: Int): Int {
        ...
        if (pressureOption < 0 || pressureOption >= pressureUnitsToSelect.size) {
            return -1
        }
        val minMaxPressureValue = when (pressureOption) {
    ...
    @Composable
    fun updatePressureUnit(pressureOption: Int): String {
        val localcontext = LocalContext.current
        val pressureUnitsToSelect =
            localcontext.resources.getStringArray(R.array.pressure_units)
        ...
        // Get the chosen unit
        val chosenUnit = pressureUnitsToSelect[pressureOption]
        return chosenUnit

    }

    @Composable
    fun updatePressure(pressureValue: Int, pressureOption: Int): Int {
        val localcontext = LocalContext.current
        val pressureUnitsToSelect =
            localcontext.resources.getStringArray(R.array.pressure_units)
        ...
        val convertedPressure = when (pressureOption) {
